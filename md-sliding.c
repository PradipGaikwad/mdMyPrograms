...
void anagram() 
{
    ...
    for(winend=0; winend < strlen(s1); winend++) {
        
        int lchar = s1[winend] - 'a';
        h1[lchar] +=1;

        if(winend >= strlen(s2)-1) {
            if(cmphash(h1,h2)) {
                printf("anagram present between %d - %d \n", winstart, winend);
//                return ;
            }

            int rchar = s1[winstart] - 'a';
            h1[rchar] -= 1;
            winstart++;
        }
    }
...
void smallWindow() 
{
    ...
    printf("smallest window for %d to %d, len %d \n", winstart, winend, minlen);
    return ;
}
